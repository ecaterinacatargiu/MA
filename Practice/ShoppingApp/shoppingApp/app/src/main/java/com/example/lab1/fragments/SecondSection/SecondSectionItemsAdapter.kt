...
class SecondSectionItemsAdapter: RecyclerView.Adapter<SecondSectionItemsAdapter.MyViewHolder>() {
    ...
    override fun onBindViewHolder(holder: MyViewHolder, position: Int) {
        ...
        holder.itemView.entityAttr2.text = currentEntity.quantity.toString()
        holder.itemView.entityAttr3.text = currentEntity.price.toString()
    ...
    fun filter(entities: List<Entity>) : List<Entity>
    {
        var listAfter = ArrayList<Entity>()
         for(entity in entities)
         {
              if(entity.status == "needed" || entity.status == "desired")
              {
