...
class MainActivity : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val request = Request.Builder().url("ws://10.0.2.2:2021").build()
        ...
        override fun onMessage(webSocket: WebSocket?, text: String?) {
            ...
            GlobalScope.launch(Dispatchers.Main) {
                var entity = Entity(
                    ...
                    number = JSONObject(text).getString("number"),
                    address = JSONObject(text).getString("address"),
                    status = JSONObject(text).getString("status"),
                    count = JSONObject(text).getString("count").toInt()
                );

                Toast.makeText(applicationContext, "A new space was added : " + entity, Toast.LENGTH_LONG).show()
            }
        }
